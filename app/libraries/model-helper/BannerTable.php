...
class BannerTable  extends TableHelper
{
	private $query;
	private $table 	= "posts";
    ...
    public function column_post_title( $item ) 
    {
    	...
    	return sprintf(
            '<a href="banner/%s/edit">%s</a>
        	 <div class="row-actions">
	            <span class="edit">
	                <a href="banner/%s/edit" title="Edit this item">Edit</a> |
	            </span>
	            <span class="view">
	                <a href="%s" target="_blank" title="Delete " rel="permalink">View</a> | 
	            </span>
	            <span class="delete">
	                <a href="banner/delete?cid[]=%s" class="confirm-delete" title="Delete " rel="permalink">Delete</a>
	            </span>
	         </div>
	...
	{	
		...
		$this->query = "SELECT 
				p.id, 
				p.guid,
				(SELECT GROUP_CONCAT(`name` SEPARATOR ',') FROM ".$tablPrefix."term_taxonomy as tt 
					JOIN ".$tablPrefix."terms as t ON tt.term_id = t.term_id
					JOIN ".$tablPrefix."term_relationships as tr ON tt.term_taxonomy_id = tr.term_taxonomy_id 
					WHERE tr.object_id = p.id) as category,
				p.post_title, 
