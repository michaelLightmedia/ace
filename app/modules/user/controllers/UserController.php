...
class UserController extends \Controller
{
            ...
            if($validator->passes())
            {
                ...
                if($user->save())
                {
                    ...
                    \Mail::send('user::emails.register',$data, function($message) use ($user){
                        $message->to($user['email'], $user['name'])->subject('Welcome to Sitetbo');
                    });
    ...
    /**
	 * Display current customer purchases
	 *
	 *@param none
	 *@return none
	 */
    ...
    public function customerOrdersAction()
    {
		...
		$args = array(
			'columns' => array(
			'order_no'		=> 'Order no.',
			'total_item'	=> 'Total Quantity',
			'total_price' 	=> 'Total Price',
			'total_points_to_earn' 	=> 'Total points to earn',
			'redeemable_points' 	=> 'Maximum redeemable csk$',
			'customer_redeemed_points' 	=> 'Customer points used',
			'date_order' 			=> 'Date of order',
			'payment_status' 		=> 'Payment Status',
			'order_status' 			=> 'Order Status'
			),
			'urlDetails' => 'customer/profile/purchases?order_id=%s',
		);

		$lists = new \OrderTable( $user_id, $args );

		if(\Request::ajax())
		{

			$html = $lists->ajaxTableHelper($lists, FALSE);
    	...
    	return \View::make('user::customer.orders')
    		->with('lists', $lists)
    		->with('user', $user);
    }

    /**
	 * Display current customer orders
	 *
	 *@param none
	 *@return none
	 */

    public function customerPurchasesAction( )
    {
    	$order_id = Input::get('order_id');

    	$user_id = \Auth::User()->id;
    	$lists = new \OrderDetailsTable($order_id, $user_id );
    	...
    	return \View::make('user::customer.purchases')
    		->with('lists', $lists)
    		->with('user', $user);
    }

    /**
	 * Display current customer friends
	 *
	 *@param none
	 *@return none
	 */
    ...
    public function customerPointsAction()
    {
    	...
    	return \View::make('user::customer.points')
    		->with('lists', $lists)
    		->with('user', $user);
    }
    ...
    public function customerFriendsAction()
    {
    	$user_id = \Auth::User()->id;
		$user = \User::find( $user_id );
    	...
    	return \View::make('user::customer.friends')
    		->with('lists', $lists)
    		->with('user', $user);
    }
    ...
    public function userPurchasesAction($id)
    {
    	$lists = new \OrderDetailsTable(false, $id);
		...
		return  \View::make('user::user.purchases')
					->with('user', $user)
					->with('groups', $groupArr)
					->with('lists', $lists);
    ...
    public function userOrdersAction( $id )
    {
		...
		return  \View::make('user::user.orders')
					->with('user', $user)
					->with('groups', $groupArr)
					->with('lists', $lists);
    ...
    public function userPointsAction( $id )
    {
    	...
    	return  \View::make('user::user.points')
    		->with('lists', $lists)
    		->with('user', $user);
    }
    ...
    public function userFriendsAction($id)
    {
    	...
    	return  \View::make('user::user.friends')
    			->with('lists', $lists)
    			->with('user_id', $id);
    }
    ...
    public function deleteFriendsAction()
    {
    	if(\Request::ajax())
		{
			...
			$queryResult = \DB::table('posts_shared')->whereIn( 'id', $cid )->delete();
